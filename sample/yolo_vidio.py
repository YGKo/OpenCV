...
while True:
   ...
   # cv2.imshow("gray", gray)
    ...
    gray = cv2.threshold(img1, 127, 255, cv2.THRESH_BINARY_INV)[1]
 ...
 #   gray = cv2.medianBlur(gray, 10)
    ...
    if k == 27:
        break
    print('OCR', ocr_boxes.splitlines())
    print(ocr_text)
    ocr_text = ocr_text.split('\n')
    print(ocr_text.split(), )
    ocr_text = [ocr_text.split() for letter in ocr_text]
    for letter in ocr_text:
        cv2.rectangle(img1, (int(letter[1]), h - int(letter[2])), (int(letter[3]), h - int(letter[4])),
                      (0, 0, 255), 1)
