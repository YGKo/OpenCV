...
now = datetime.now()
print('===Gaussian Blur 필터를 적용 후 엣지 검출=====')
print(now)
#Gaussian Blur 필터를 적용 후 엣지 검출
...
#contour의 가로 세로 비율 값과 면적을 통해, 번호판 영역에 벗어난 걸로 추정되는 값
now = datetime.now()
print('===번호판 후보 영역 선정=====')
print(now)
...
#Rectangle과 Rectangle사이 간격과 기울기 값이 일정 범위카운터 추출
now = datetime.now()
print('===번호판 영역 판별=====')
print(now)
MIN_AREA = 80
...
for i, plate_img in enumerate(plate_imgs):
    ...
    pytesseract.pytesseract.tesseract_cmd = r'C:\Program Files (x86)\Tesseract-OCR\tesseract'  # window10 PC OCR 엔진 실행
